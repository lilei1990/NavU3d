 ...
 * 时间 : 2021/02/26.
 * 邮箱 :416587959@qq.com
 * 描述 :LivaData来实现数据传递,可用来替换EventBus
 */
...
public class LiveDataBus {
     ...
     * @param type 订阅的数据类型
     * @param <T> 泛型
     * @return 返回订阅的livedata
     */
    public <T> MutableLiveData<T> with(Enum key, Class<T> type) {
        return with(key.name(),type);
    }
    public MutableLiveData<Object> with(String key) {
        return with(key, Object.class);
    }

    public MutableLiveData<Object> with(Enum key) {
        return with(key, Object.class);
    }
