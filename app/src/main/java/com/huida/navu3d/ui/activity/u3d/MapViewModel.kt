...
//        B=mCurrenLocation
    ...
    fun DerwLeftLine(aMap: AMap) {
        ...
        for (i in 0..50) {
            ...
            val latLngs: MutableList<LatLng> = ArrayList()
            val pathStart = polyline.getPoint(0)
            val pathEnd = polyline.getPoint(1)
            A = LatLng(pathStart.x, pathStart.y)
            B = LatLng(pathEnd.x, pathEnd.y)
            latLngs.add(A)
            latLngs.add(B)
            aMap.addPolyline(
                PolylineOptions().addAll(latLngs).width(10f).color(Color.argb(255, 1, 255, 1))
...
//        binding.map.map.uiSettings.isZoomGesturesEnabled = false
            mLocationStyle.postValue(myLocationStyle)
        }
...
//                LogUtils.e("${p0.latitude},${p0.longitude}")
     ...
     * 停止定位服务
     */
    fun stopLoaction(context: Context) {
        location.deactivate()
    }
     ...
     * 添加覆盖物
     */
    fun getMarker(u3dActivity: U3dActivity) {
        val options = MarkerOptions()
        options.draggable(true)
            .snippet("DefaultMarker")
    ...
    fun getCarMarker(aMap: AMap) {
        ...
        moveMarker.setTotalDuration(80) //设置平滑移动的总时间
        moveMarker.setMoveListener {
            Log.i("MY", "distance:  $it")
        }
